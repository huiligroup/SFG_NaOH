...
class VisNetEIP(nn.Module):
    ...
    def extract_atom_features(
        ...
        edge_index: Tensor | None = None,
        cell_shift: Tensor | None = None,
        include_vector_norm: bool = False,
    ) -> dict[str, Tensor]:
        """Return atom descriptors from the representation before the energy head.

        ``atom_features`` is the scalar ViSNet representation ``x`` that is fed
        into ``output_model.pre_reduce`` during energy prediction. It is the
        default descriptor used by the molecule-selection scripts.
        """

        self.pbc_distance.set_graph(cell=cell, edge_index=edge_index, cell_shift=cell_shift)
        x, v = self._representation(z, pos, batch)
        result = {"atom_features": x}
        if include_vector_norm:
            result["vector_norm"] = torch.linalg.norm(v, dim=1)
        return result
